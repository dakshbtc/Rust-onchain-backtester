/*!
Mean Reversion Strategy Test Suite
==================================

This test file contains all configurable parameters for testing the Mean Reversion Strategy.
...
use rust_backtester::strategies::MeanReversionStrategyModified;
...
// Mean Reversion Strategy Configuration (universal parameters imported from common_config)
const DEFAULT_WINDOW_SIZE: usize = 400; // Default rolling window for mean reversion
const DEFAULT_BUY_THRESHOLD: f64 = 1.5; // Buy when Z-score < -3.5 (oversold)
const DEFAULT_BUY_VOLUME_THRESHOLD: f64 = 2.0; // Buy when volume Z-score < 3.0 
...
const VOLUME_COLUMN: &str = "Token Amount"; // Column to use for volume calculation
const MAX_BUYS: usize = 1; // Maximum number of buy positions per token (increased for DCA)
// Removed: min transactions between buys (no longer used)

// Mean Reversion Strategy Override
const USE_DYNAMIC_FEES_OVERRIDE: bool = true; // Override common config for mean reversion

fn test_mean_reversion_strategy() -> Result<()> {
	println!("{}", "=".repeat(80));
	println!("📊 Testing Mean Reversion Strategy");
	println!("{}", "=".repeat(80));

	// Create strategy instance with configured parameters (verbose=true for single test)
	let mut strategy = MeanReversionStrategyModified::new(
		DEFAULT_WINDOW_SIZE,
		DEFAULT_BUY_THRESHOLD,
		DEFAULT_BUY_VOLUME_THRESHOLD,
		...
		VERBOSE, // verbose=true from common config
	);
	...
	println!(
		"  🎯 Strategy: Pure mean reversion - Z-score entry and exit with 65% stop loss"
	);
	println!();
	...
	println!("\n--- Mean Reversion Strategy Debug Statistics ---");
	println!(
		"Total buy signals: {}",
...
fn main() -> Result<()> {
	println!("🚀 Starting Mean Reversion Strategy Testing Suite");
	println!("Based on the exact logic from gemini-mean-reversion.py");
	println!();
...
use super::*;
	...
	fn test_strategy_creation() {
		let strategy = MeanReversionStrategyModified::new(
			DEFAULT_WINDOW_SIZE,
			DEFAULT_BUY_THRESHOLD,
			DEFAULT_BUY_THRESHOLD,
			DEFAULT_SELL_THRESHOLD,
	...
	fn test_framework_creation() {
		let framework = BacktestFramework::new(
			...
			false, // verbose = false for tests
			USE_DYNAMIC_FEES_OVERRIDE, // use_dynamic_fees override
		);
