...
use super::helpers::{can_buy_with_constraints, can_sell_with_hold_blocks, get_block_id, get_column_value, get_boolean_column_value, get_sol_in_curve, update_threshold_tracking, get_timestamp_ms};
...
pub struct SignedTradeVelocity {
	// Strategy parameters
	window_size: usize,
	buy_threshold: f64,
	sell_threshold: f64,
	...
	sell_trade_history: HashMap<String, VecDeque<i64>>, // timestamps of sell trades
	buy_trade_history: HashMap<String, VecDeque<i64>>,  // timestamps of buy trades
	velocity_window_seconds: f64, // Time window for velocity calculations
	sell_velocity_buy_trigger_threshold: f64, // e.g., 3.0 sells/sec triggers contrarian buy
	sell_buy_ratio_threshold: f64, // e.g., 3.0 = 3x more sells than buys
	...
	take_profit_triggers: i64,

	// Output control
	verbose: bool, // Controls debug output verbosity
}

impl SignedTradeVelocity {
	...
	pub fn new(
		...
		min_hold_blocks: i64,
		min_blocks_between_sell_buy: i64,
		sell_velocity_buy_trigger_threshold: f64,
		sell_buy_ratio_threshold: f64,
		verbose: bool,
	) -> Self {
		...
		Self {
			...
			sell_trade_history: HashMap::new(),
			buy_trade_history: HashMap::new(),
			velocity_window_seconds: 60.0, // 60 second window for velocity calculations
	...
	/// Calculate sell trade velocity (sells per second)
	fn calculate_sell_velocity(&self, token_address: &str, current_timestamp_ms: i64) -> f64 {
		let sell_hist = match self.sell_trade_history.get(token_address) {
			Some(hist) => hist,
			None => return 0.0,
		};
		
		if sell_hist.is_empty() {
			return 0.0;
		}
		...
		let mut sells_in_window = 0;
		for &timestamp in sell_hist.iter().rev() {
		...
		sells_in_window as f64 / self.velocity_window_seconds
	}

	/// Calculate buy trade velocity (buys per second)
	fn calculate_buy_velocity(&self, token_address: &str, current_timestamp_ms: i64) -> f64 {
		let buy_hist = match self.buy_trade_history.get(token_address) {
			Some(hist) => hist,
			None => return 0.0,
		};
		
		if buy_hist.is_empty() {
			return 0.0;
		}
		...
		let mut buys_in_window = 0;
		for &timestamp in buy_hist.iter().rev() {
			if timestamp >= window_start_ms {
				buys_in_window += 1;
			} else {
				break;
		...
		buys_in_window as f64 / self.velocity_window_seconds
	}

	/// Calculate sell/buy ratio (sell pressure indicator)
	fn calculate_sell_buy_ratio(&self, token_address: &str, current_timestamp_ms: i64) -> f64 {
		let sell_velocity = self.calculate_sell_velocity(token_address, current_timestamp_ms);
		let buy_velocity = self.calculate_buy_velocity(token_address, current_timestamp_ms);
		
		if buy_velocity == 0.0 {
			return if sell_velocity > 0.0 { f64::INFINITY } else { 0.0 };
		}
		
		sell_velocity / buy_velocity
	}
...
impl TradingStrategy for SignedTradeVelocity {
	fn should_buy(
		&mut self,
		token_address: &str,
		current_price: f64,
		...
		// === NEW: Contrarian Buy Signal Based on Sell Pressure ===
		let _sell_velocity = self.calculate_sell_velocity(token_address, current_timestamp_ms);
		let _sell_buy_ratio = self.calculate_sell_buy_ratio(token_address, current_timestamp_ms);
		...
		println!("Sell buy ratio: {}", _sell_buy_ratio);
		println!("Sell velocity: {}", _sell_velocity);
		println!("Sell buy ratio threshold: {}", self.sell_buy_ratio_threshold);
		println!("Sell velocity buy trigger threshold: {}", self.sell_velocity_buy_trigger_threshold);
		// Buy signal: Z-score below threshold
		if  _sell_velocity > self.sell_velocity_buy_trigger_threshold {
			self.total_buy_signals += 1;
			// Verbose logging enabled for comparison
			if self.verbose {
				println!(
					"\n[Index {}] 📈 INTER TRADE BUY for {}",
					current_index,
	...
	fn should_sell(
		...
		for (i, buy_price) in buy_prices.iter().enumerate() {
			...
			let profit_percentage = (current_price - buy_price) / buy_price;
			if profit_percentage >= 0.3 {
				...
				return Ok((true, SellReason::TakeProfit(profit_percentage)));
			} else
	...
	fn update_data(
		...
		// Initialize direction tracking for new tokens
		let max_history = (self.velocity_window_seconds * 10.0) as usize; // Keep 10x window
		if !self.sell_trade_history.contains_key(token_address) {
			self.sell_trade_history.insert(
				token_address.to_string(),
				VecDeque::with_capacity(max_history),
			);
			self.buy_trade_history.insert(
				token_address.to_string(),
				VecDeque::with_capacity(max_history),
			);
		}

		let is_buy_trade = get_boolean_column_value(row_data, "is_buy");

		// Track the trade
		if is_buy_trade {
			let buy_hist = self.buy_trade_history.get_mut(token_address).unwrap();
			buy_hist.push_back(current_timestamp_ms);
			if buy_hist.len() > max_history {
				buy_hist.pop_front();
			}
		} else {
			let sell_hist = self.sell_trade_history.get_mut(token_address).unwrap();
			sell_hist.push_back(current_timestamp_ms);
			if sell_hist.len() > max_history {
				sell_hist.pop_front();
			}
		}
	...
	fn get_strategy_name(&self) -> String {
		format!(
			"Inter Trade Velocity w/ {} SOL Threshold Gate [Z-score on: {}] (max_buys: {}, hold_blocks: {}, sell_to_buy_blocks: {}, buy_to_buy_blocks: {})",
			self.min_sol_in_curve,
			self.z_score_column,
			self.max_buys,
			self.min_hold_blocks,
