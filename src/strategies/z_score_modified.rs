...
use super::helpers::{can_buy_with_constraints, can_sell_with_hold_blocks, get_block_id, get_column_value, get_sol_in_curve, update_threshold_tracking, get_volume_column_value};
...
pub struct MeanReversionStrategyModified {
	// Strategy parameters
	window_size: usize,
	buy_threshold: f64,
	...
	z_score_data_history: HashMap<String, VecDeque<f64>>, // History for z-score column values
	volume_data_history: HashMap<String, VecDeque<f64>>, // Separate history for volume values
...
impl MeanReversionStrategyModified {
	...
	pub fn new(
		...
		Self {
			...
			volume_data_history: HashMap::new(),
	...
	/// Helper to compute z-score from any provided history deque
	fn compute_z_from_history(&self, history: &VecDeque<f64>, current_value: f64) -> Option<f64> {
		...
		Some((current_value - rolling_mean) / rolling_std)
	}

	/// Calculate the current Z-score for the configured z-score column (optimized)
	fn calculate_z_score(&mut self, token_address: &str, current_value: f64) -> Option<f64> {
		let history = match self.z_score_data_history.get(token_address) {
			Some(history) => history,
			None => return None,
		};
		let result = self.compute_z_from_history(history, current_value);
		if result.is_some() { self.z_score_calculations += 1; }
		result
	}

	/// Calculate the current Z-score for volume using its own history
	fn calculate_volume_z_score(&mut self, token_address: &str, current_value: f64) -> Option<f64> {
		let history = match self.volume_data_history.get(token_address) {
			Some(history) => history,
			None => return None,
		};
		let result = self.compute_z_from_history(history, current_value);
...
impl TradingStrategy for MeanReversionStrategyModified {
	fn should_buy(
		&mut self,
		token_address: &str,
		current_price: f64,
		...
		let vol_z_score = match self
			.calculate_volume_z_score(token_address, current_volume)
		{
			Some(z_score) => z_score,
			None => return Ok(false),
		};
		...
		println!("Volume Z-score: {:.3}", vol_z_score);
		println!("Buy threshold: {:.3}", self.buy_threshold);
		println!("Buy volume threshold: {:.3}", self.buy_volume_threshold);
		// Buy signal: Z-score below threshold
		if z_score > self.buy_threshold && vol_z_score > self.buy_volume_threshold {
	...
	fn update_data(
		...
		// Initialize VecDeque for new tokens (for z-score and volume calculations)
		...
		if !self.volume_data_history.contains_key(token_address) {
			let max_history = self.window_size + 500; // Keep some extra for robustness
			self.volume_data_history.insert(
				token_address.to_string(),
				VecDeque::with_capacity(max_history),
			);
		}

		// O(1) append operation with automatic eviction of old data (z-score column)
		...
		// O(1) append for volume history
		let current_volume = get_volume_column_value(row_data, &self.volume_column);
		let vol_history = self.volume_data_history.get_mut(token_address).unwrap();
		if vol_history.len() >= self.window_size + 500 {
			vol_history.pop_front();
		}
		vol_history.push_back(current_volume);
	...
	fn get_debug_stats(&self) -> HashMap<String, i64> {
		...
		stats.insert("take_profit_triggers".to_string(), self.take_profit_triggers);
		stats.insert(
	...
	fn get_system_name(&self) -> String {
		"ModifiedMeanReversionStrategy".to_string()
	}
